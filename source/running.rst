...
Specify what type of a backup you wish to make: physical or logical. 
...
The following diagram illustrates the logical backup flow.

.. image:: _images/pbm-backup-shard.png
   :alt: Logical backup in sharded clusters
|
...
As of version 2.0, the ``pbm list`` output includes the type of backup.
...
restore. |PBM| identifies the type of the backup (physical or logical) and restores the database  up to the backup completion time (available in :ref:`pbm list output <pbm.running.backup.listing>` as of version 1.4.0).
...
.. rubric:: Memory consumption adjustment
...
.. note::
   ...
   Physical backup information is not available in pbm logs. 
